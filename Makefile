...
config: hetzner_ddns.json
	mkdir -p $(prefix)/usr/local/etc
	install -m 0644 -p hetzner_ddns.json $(prefix)/usr/local/etc/hetzner_ddns.json.sample
	test -f $(prefix)/usr/local/etc/hetzner_ddns.json || \
		install -m 0640 -p hetzner_ddns.json $(prefix)/usr/local/etc/hetzner_ddns.json
...
openwrt-rc: hetzner_ddns.openwrt.rc
	mkdir -p $(prefix)/etc/init.d
	install -m 0755 -p hetzner_ddns.openwrt.rc $(prefix)/etc/init.d/hetzner_ddns
...
remove:
	rm -f 	$(prefix)/usr/local/bin/hetzner_ddns \
		$(prefix)/usr/local/etc/hetzner_ddns.json.sample \
